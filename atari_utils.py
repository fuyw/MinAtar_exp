...
class FrameStack:
    ...
    @property
    def action_space(self):
        return self.preproc.action_space
...
def create_vec_env(game: str, num_envs: int, clip_rewards: bool = False, seeds: List[int] = None):
    def create_env():
        env = gym.make(game)
        # if clip_rewards:
    ...
    envs.seed(seeds)
    stack = VecFrameStack(envs, num_frames=4, num_envs=num_envs)
    return stack
